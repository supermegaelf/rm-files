...
install_system_packages() {
    ...
    if ! ufw allow 22/tcp comment 'SSH' > /dev/null 2>&1 || ! ufw allow 443/tcp comment 'VK CDN origin (caddy XHTTP)' > /dev/null 2>&1 || ! ufw --force enable > /dev/null 2>&1; then
...
ensure_ws_inbound_in_profile() {
    ...
    if echo "$config" | jq -e --arg tag "$WS_INBOUND_TAG" '.inbounds[] | select(.tag == $tag)' > /dev/null 2>&1; then
        ...
        local existing_path
        existing_path=$(echo "$config" | jq -r --arg tag "$WS_INBOUND_TAG" '.inbounds[] | select(.tag == $tag) | .streamSettings.xhttpSettings.path // empty')
        RANDOM_PATH=$(printf '%s' "$existing_path" | sed 's|^/||')
        if [ -z "$RANDOM_PATH" ]; then
            echo -e "${RED}${CROSS}${NC} Failed to recover XHTTP path from existing inbound"
            exit 1
        fi
        resolve_ws_inbound_uuid
    ...
    echo -e "${GRAY}  ${ARROW}${NC} Generating XHTTP path"
...
create_cdn_host_in_panel() {
    ...
    host_data=$(jq -n \
        ...
        '{
            ...
            securityLayer: "TLS",
            isDisabled: false,
            xhttpExtraParams: { uplinkHTTPMethod: "GET" },
            inbound: {
...
continue_by_instruction() {
    echo -e "${YELLOW}${WARNING}${NC} Continue with xhttp-vk-cdn.md"
    pause_step
}
